...
> 当前实现状态：`目标合同已定，迁移未完成`
> 实现说明：本基线给出 Orchestration / Lifecycle 的最小实现单元，不把现有旧代码描述成目标事实。

## 1. 控制目标

让实现者无需自行发明产品规则，即可把旧的单次运行和独立收敛路径迁移为一个长期 Pi Agent 主链。
...
7. 以停写、导入、哈希校验和按 Workspace 原子切换迁移旧消息，不长期双写。
8. 完成 Trace 关联与端到端验收后再删除兼容字段。

迁移期间旧字段只能作为兼容输入，不能继续产生新的目标状态语义。
...
- 迁移校验失败：原子切换前恢复旧路径；切换后已有新事实则只允许修复前进或再次受控迁移。
...
1. 搜索生产路径不存在旧运行类型的主动调用。
2. 真实端到端流程中同一 Pi 完成读取、确认、提交和回复。
3. 提交后投影失败再恢复，无重复 Revision 或语义重跑。
4. 旧客户端兼容请求不能绕过新提交器直接写稳定状态。
5. 迁移开关关闭旧路径后，完整回归仍通过。
