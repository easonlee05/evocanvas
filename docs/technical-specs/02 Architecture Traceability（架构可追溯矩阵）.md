...
> 当前结论：`十二项 Harness 均为 L3；目标合同已定不等于代码已完成`

## 1. 文档目的

本矩阵防止四类混淆：把主 PRD 体验规则当技术接口、把当前旧代码当目标架构、把 Harness L3 当实现完成、把 Pi 已有技术能力当 EvoCanvas 产品规则已经落地。
...
| `部分接入` | 当前代码已使用部分目标底座，但主链未闭环 |
| `代码待迁移` | 目标合同明确，当前生产路径仍使用旧结构 |
| `待验证` | 已有实现，但缺少真实入口或端到端证据 |
| `历史参考` | 仅用于识别迁移债务，不可作为新实现依据 |
...
| Memory | [`Memory and State`](<../harness/02-memory-state/00 Memory and State（记忆与状态）.md>) | Pi Session + 完整不可变 Revision | 当前存在旧运行记录和状态存储，统一 Revision 待落地 |
...
| Tools | [`Tool Contract`](<../harness/03-runtime-tools/02 Tool Contract（工具契约）.md>) | Pi Tool Loop + EvoCanvas 工具 | 当前 Tool Gateway 按旧运行类型授权；身份分层和 `replay` 策略待迁移 |
...
| Lifecycle | [`Lifecycle`](<../harness/04-orchestration-lifecycle/02 Lifecycle（生命周期）.md>) | Pi 技术状态 + Revision / Handoff 状态 | 旧产品运行状态和水位待停止新写入 |
...
| Primary Pi Session | 主 PRD 右侧 AI 助手 | Context、Runtime | Session Entry / Technical Trace | 代码待迁移 |
| Workspace–Session Binding | 主 PRD 工作区连续对话 | Runtime、Lifecycle | `binding / ready / unavailable / archived` | 代码待迁移 |
| Instruction / Skill Resources | 主 PRD 对话塑形 | Instructions | 制品版本与使用 Trace | 部分接入 |
| Workspace Context Snapshot | 主 PRD 当前工作上下文 | Context | 临时稳定工作快照 | 代码待迁移 |
| Workspace Artifact / Structured Work Package | 主 PRD 结构收敛 | Memory、Lifecycle、Governance | 完整不可变 Revision | 代码待迁移 |
| Semantic Commit | 主 PRD 用户确认与直接编辑 | Tools、Governance、Verification | Commit Result / new Revision | 代码待迁移 |
| Lifecycle Coordinator | 主 PRD 工作区状态与删除反馈 | Lifecycle、Safety | close/archive/replace/delete outcome | 代码待迁移 |
| Canvas Renderer | 主 PRD 画布显影 | Observability、Verification | Projection Checkpoint | 部分接入，目标链待迁移 |
| Handoff Renderer | 主 PRD 结构化交接 | Memory、Governance | confirmed handoff view | 代码待迁移 |
...
若当前代码或历史技术文档出现这些内容，迁移前保留兼容读取，停止新增依赖，并以 Harness L3 和本矩阵为目标。

## 7. 当前实现证据

已确认当前仓库：

- `pi-runtime/package.json` 使用 `@earendil-works/pi-agent-core`、`@earendil-works/pi-ai` `0.84.1`，telemetry override 为 `0.84.2`，尚未统一到目标 `0.85.1`；
- `pi-runtime/src/runtime/executor.ts` 仍直接创建低层 `Agent`；
- `pi-runtime/src/server.ts` 仍暴露 chat / judgement / convergence 运行类型；
- `app/canvas/pi_kernel.py` 仍组装外部请求、判断和独立收敛运行；
- `app/canvas/agent_execution/contracts.py` 与 TypeScript contracts 仍含旧 Context Manifest 和运行 DTO；
- 当前代码尚不能证明 Workspace–Primary Session、统一语义提交和 Revision 投影闭环已完成。

这些证据只说明迁移状态，不改变目标架构。

## 8. 进入实现完成的硬门

1. 生产入口不再主动调用旧 judgement / convergence 路径。
...
8. 旧数据完成迁移、兼容读取和回退验证后，才删除旧字段。
9. `submission_id / entry_id / operation_id / invocation_id` 分责，工具重放与结果状态按合同工作。
10. 迁移无长期双写，close/archive/replace/delete 的恢复和终态均通过测试。

## 9. 当前结论

Harness 方法成熟度已经收敛到 L3；目标架构和迁移方向已经明确。当前仓库仍处于“部分接入 / 代码待迁移”，不得用文档 L3 或包版本号宣称 Pi 内核重构已完成。
