...
它是一张面向产品经理的持续演化工作画布，用来接住多源输入、暴露不确定性、沉淀约束、形成待决策，并最终收束为结构化交接物。
...
### 11.3.1 轻手绘卡片骨架（当前问题卡 / 证据卡共用基底）

问题卡与证据卡已经验证出一套可复用的轻手绘卡骨架，后续待澄清卡、规则卡、交接卡如需延续当前 UI kit 语言，应优先复用这一骨架，而不是重新发明单卡结构。

建议统一为五层：

1. 外轮廓层
   一层主描边 + 一层极淡偏移描边，营造稳定但不呆板的纸面轮廓。
2. 顶部信息层
   左侧是类型图标与类型名，右侧是状态 / 语义标签与更多操作。
3. 主内容层
   承接业务标题与摘要，是卡片的第一阅读区。
4. 内框内容层
...
1. 外轮廓必须只包整张卡，不再叠加系统矩形选中框。
2. 内框只包“结构化内容区”，不能把引用数、评论数、来源行或进度条卷进去。
3. 顶部信息层与主内容层之间不插入额外横线。
4. 协作尾部层与上方内容之间允许一条极淡分隔线，但不允许再套容器边框。

### 11.3.1.1 主题色规则（当前问题卡 / 证据卡已验证）

问题卡与证据卡最新一轮打磨得到的结论是：卡片不再只靠标签表达类型，而应建立“类型图标就是主题色源”的规则。

也就是说，每张卡都应从自己的类型图标里提取一套轻主题色，而不是所有卡都共用一套黑边框或统一强调色。

统一规则如下：

1. 类型图标颜色就是该卡片的主题色起点。
2. 外轮廓描边、偏移描边、内框描边、内框浅底，都应从这个主题色衍生。
3. 同一张卡内，颜色关系应满足：
   图标最实，外框次之，内框描边更浅，内框底色最浅。
4. 主题色只负责“识别与气质”，不负责替代选中态。
5. 选中态与悬停态仍统一使用系统蓝，避免把“卡片类型”与“交互状态”混淆。

当前已验证的落地方式：

1. 问题卡：
   跟随问题图标的珊瑚红系。
   外轮廓不是纯黑，也不是暗红棕，而是偏活力的浅红铅笔线。
   内框描边与浅底继续沿用这套红系，但必须保持非常淡，接近米白底上的轻红提示。
2. 证据卡：
   跟随证据图标的蓝系。
   外轮廓、内框、弱强调都围绕这套蓝色展开，用作主题识别示例。

不建议：

1. 问题卡外框继续使用中性黑，而标签单独用红。
2. 证据卡是蓝系，问题卡却仍是黑系，导致整套卡片主题逻辑断裂。
3. 把主题色直接铺成高饱和大底，让卡片变成强情绪块。

### 11.3.2 标题规则（问题卡 / 证据卡）

当前 UI kit 落地结论是：问题卡与证据卡都属于“内容对象层”的核心卡，因此标题语言需要统一，不应一张偏系统字、一张偏内容字。
...
1. 类型标题
   例如：`问题`、`证据`
2. 业务标题
   例如：`618 积分发放链路治理（v1.0）`、`活动高峰期刷分投诉与异常补发同时上升`

使用规则：

1. 类型标题固定放在左上角，紧跟类型图标。
2. 类型标题使用小赖字体（小赖手写体），表达内容对象的轻手写感。
3. 业务标题也统一使用小赖字体，不再混用系统无衬线或楷体。
4. 类型标题是“卡片类别名”，业务标题是“当前对象名”，两者不能合并成一行概念。
5. 业务标题默认允许换行展示，不应依赖省略号维持整洁。

建议层级：

1. 类型标题：20 - 22px，常规字重
2. 业务标题：18px 左右，加粗，用作卡片第一视觉焦点
3. 摘要正文：回到运行态正文字体，不继续使用手写体

### 11.3.3 标签规则（问题卡 / 证据卡）

问题卡与证据卡都已证明：标签不是“装饰 badge”，而是卡片顶层信息架构的一部分。因此，标签应统一抽象为“手绘语义签”组件，而不是继续混用系统 chip、badge、pill。
...
1. 标签全部使用手绘描边样式：
   一层主描边 + 一层极淡偏移描边。
2. 标签字体统一使用小赖字体。
3. 标签圆角为圆角矩形，不使用 SaaS 胶囊。
4. 标签不应出现重填充、强阴影或过饱和底色。
5. 标签高度统一以问题卡 `待解决` 标签为基准，不反向压矮状态标签。
6. 右上角状态标签、证据属性标签、底部语义标签，必须共享同一套高度、内边距与字高基线。
...
1. 右上角标签
   放在更多操作 `...` 左侧，与顶部信息层同一行。
2. 底部语义标签
   仅在卡片确实需要一个“结论性语义收束”时使用，例如问题卡底部的 `核心问题`。
3. 同一张卡的标签不要同时散落在标题下方和右上角，优先收束到右上角。
...
1. 问题卡：
   右上角优先放状态标签，例如 `待解决`。
2. 证据卡：
   右上角放证据属性标签，例如 `用户反馈`、`高风险`、`数据证据`。
3. 只有当顶部横向空间不足时，才允许第二行标签；默认目标是单行内收束。

### 11.3.4 布局规则（问题卡 / 证据卡）

问题卡与证据卡虽然内容不同，但布局节奏应该一致，这样用户在同一画布上切换阅读对象时不会不断重建阅读模式。
...
问题卡是“问题定义区”的主卡，信息目标是帮助用户快速收束“到底在解决什么”。

专属规则如下：

1. 右上角只放一个状态标签，例如 `待解决`。
2. 业务标题要明显强于摘要，成为卡片主焦点。
3. 内框内容优先承接：
   目标对象、动作边界、衡量标准等问题拆解条目。
4. 计数行右侧允许出现一个总结性标签，例如 `核心问题`。
5. 底部右侧更适合展示分母式进度，例如 `1 / 5`。
6. 整张卡的主题色跟随问题图标的红系：
   顶部图标、外轮廓、内框描边、内框浅底应是同一色系的深浅变化。
7. 当前推荐方向是“更有活力的浅珊瑚红”，而不是偏灰、偏棕、偏暗的旧纸张红。
...
证据卡是“探索发现区”的主卡，信息目标不是下结论，而是承接可追溯事实。

专属规则如下：

1. 右上角允许出现两个以内的证据属性标签，例如 `用户反馈`、`高风险`。
2. 业务标题仍使用小赖字体，但颜色可以偏蓝，以区别问题卡的中性黑。
3. 摘要区负责交代证据背景，不做结论性表达。
4. 内框内容优先承接：
   引用句、抽样发现、定量事实、竞品观察等短条目。
5. 计数区统一使用：
   引用 / 评论，而不是清单类图标。
...
1. 在标题下再挂一排普通 SaaS badge。
2. 用强色大底来表现证据可信度。
3. 用与问题卡完全不同的标题语言或标签语言。

### 11.3.7 后续卡片复用建议

后续重做待澄清卡、规则卡、交接卡时，建议按“同骨架、异语义”的方式推进：

1. 先复用轻手绘卡骨架。
2. 再替换类型图标、标题颜色、标签颜色与内框内容类型。
3. 最后根据业务需要微调尾部协作区。

推荐的复用顺序：

1. 待澄清卡
   最接近问题卡，只是顶部状态与内框内容不同。
2. 规则卡
   可继续复用当前骨架，但内框内容切为规则 / checkpoint。
3. 交接卡
   可沿用布局骨架，但手绘比例应比问题卡与证据卡再低一点。

结论是：

1. 问题卡与证据卡已经形成 EvoCanvas 1.0 当前最稳定的“内容对象卡片模板”。
2. 后续新增卡片时，优先在这套模板上替换语义，不再回到“每张卡一个新布局”的做法。
3. 后续新增卡片时，必须先定义“类型图标主题色”，再派生外框、内框、标签浅底等色阶，不再逐张拍脑袋定色。
