...
同一分支内按以下可审查提交推进；当前已完成第 0–8 项的实现范围，尚未提交，继续保持按结构变化拆分：
...
阶段 0 已完成：新分支、执行计划、Python ↔ Pi 接口合同、Tool Gateway 约束、运行记录边界、事件/错误/恢复规则、机器可读 Schema、13 个合同/L3 fixture 测试、旧链消费者与数据 preflight。阶段 1 已建立 `pi-runtime/` 的 ESM TypeScript 服务骨架、RunRegistry、Fake Provider、health/readiness/capabilities、三类运行端点、终态查询和取消。阶段 2 已建立 `app/canvas/agent_execution/` 的强类型合同、`AgentExecutionPort`、唯一 `PiRuntimeClient`、NDJSON/终态查询/稳定错误映射/取消，并完成 Python 跨语言边界测试。阶段 3–4 已建立 `ProductKernel` 与 `PiCanvasKernel`：Canvas HTTP 主链现在通过 `PiRuntimeClient -> Chat/Judgement/Convergence -> Verification -> Governance -> Governed State Commit`，并落盘消息序号、四类运行记录、包级租约、判断水位、CommitAttempt 和 Outbox；旧 `active_turn` 不再阻塞新的 Pi 消息。新的 Python Fake 闭环测试已覆盖提案提交、运行记录、旧锁共存和 stale 拒绝。

阶段 5 已完成：Pi Runtime 已接入 `Pi Agent Core + Pi AI` 的真实 Provider Adapter，默认统一使用 `deepseek/deepseek-v4-flash`，仍支持 `PI_PROVIDER` 与 `PI_MODEL` 覆盖，Fake 只在显式 `fake-provider` 下启用；已完成只读 Tool Gateway、HMAC Run-scoped Token、租户/工作区/会话/包范围、工具白名单、版本、调用预算、结果大小、超时和 deadline 校验；`submit_convergence_proposal` 仅在 Pi 内存捕获并经过 Schema 校验；FastAPI 内部 Gateway 已有 401/200/403 smoke 测试。Pi Runtime 默认缺少 `DEEPSEEK_API_KEY` 时仍明确返回 `not_ready`，不会静默回退 Fake。

阶段 6 已补齐共享 `runtime_inputs` fixture、Gateway 错误码透传、消息范围绑定的 Run-scoped Token、前端响应适配和旧 Provider/WorkflowEngine 的默认懒加载隔离；前端生产构建、Python Pi/API smoke 和 TypeScript 逻辑测试已通过。当前仍缺真实 OpenAI/Anthropic 凭证联调、可监听环境下的完整 HTTP E2E、正式 Context Assembly 压缩/预算矩阵、浏览器级视觉验证，以及旧 `AgentRuntime`/`active_turn` 入口的最终清理。

下一步：在配置真实 Provider 后完成 Chat、工具往返、结构化判断、收敛提案和故障场景联调；随后按消费者和数据 preflight 清理旧在线入口。旧模块目前仍是迁移资产，不得重新成为 Canvas 主链回退。
