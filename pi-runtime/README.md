...
生产画布回合由 `submissions → turns → Primary Pi Session → workspace.propose / workspace.commit → Revision → Canvas` 驱动。Python 入口不参与生产聊天决策；Chat/Judgement/Convergence 合同被隔离在辅助边界内，不调度这条主链。
...
- 画布和消息仓储是可重建投影缓存。交接正文由已确认 Revision 派生，过时或失效交接不再作为默认输出。
...
`tests/test_canvas_turn_flow.py` 显式使用测试内的领域适配器，覆盖辅助边界，不代表新生产主链已跑通。

## 仍需明确的原生闭环与验证边界

- 原有 JSON Revision 在启动时导入事务库，原文件保留；Python 工作区若存在尚未进入 Revision 的卡片，返回 `workspace.migration_required` 并保留数据，不静默覆盖。历史工作区需单独核对来源和确认记录。
- 单个 Session 仅支持一个 Runtime 宿主写者；未实现跨宿主路由。进程中断后若已存在未完成的 Agent 消息，返回 `runtime.resume_unsupported`，不盲目重放未知工具效果。
- 当前工具循环由 Pi Agent 驱动，尚未接入完整 AgentHarness 的压缩、Steering 和 Deferred 恢复能力。辅助运行接口的取消能力不等于新 Session 已支持全部取消与恢复入口。
