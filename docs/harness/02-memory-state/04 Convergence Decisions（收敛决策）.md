...
为实现和后续文档提供一组已冻结的架构约束，防止原生主链重新引入第二套记忆、状态或交接真相源。
...
23. 历史消息导入按 Workspace 短暂停写、内容哈希校验和绑定原子切换；不建立长期双写。
24. 切换前可以放弃新 Session 并恢复原入口；切换后产生新 Entry 或 Revision 后不得回到兼容入口继续写，只能修复前进或再次受控导入。
...
本基线只能在新证据证明某项决定无法满足 Harness 最高目标时重新打开。重新打开必须记录：受影响决策、反证、替代方案、兼容影响和新的确认；不得通过代码先行或术语替换静默改变。

## 6. 失败与恢复

- 实现方案与本基线冲突：先标记实现差距，不以现有代码覆盖目标决策。
